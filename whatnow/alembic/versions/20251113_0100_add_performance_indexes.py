...
def upgrade() -> None:
    ...
    op.create_index("ix_calendar_events_time_range", "calendar_events", ["start_time", "end_time"])

    # Index on GitHub tasks iteration_title for filtering current iteration
    op.create_index("ix_github_tasks_iteration", "github_tasks", ["iteration_title"])

    # Index on pings todo_id for faster activity resolution
    op.create_index("ix_pings_todo", "pings", ["todo_id", "todo_type"])

    # Index on local_todos is_active for filtering active todos
    op.create_index("ix_local_todos_active", "local_todos", ["is_active"])

    # Index on work_sessions end_time for finding active sessions
    op.create_index("ix_work_sessions_end_time", "work_sessions", ["end_time"])
