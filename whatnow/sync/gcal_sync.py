...
from .gcal_credentials import GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, REDIRECT_URI, SCOPES
    ...
    def _fetch_events(
        ...
        if sync_token:
            ...
            return (
                self.service.events()
                .list(
                    calendarId=calendar_id, syncToken=sync_token, maxResults=100, singleEvents=True
                )
                .execute()
            )
        else:
                ...
                try:
                    ...
                    if sync_token:
                        try:
                            events_result = self._fetch_events(
                                calendar_id=calendar_id, sync_token=sync_token
                            )
                        except HttpError as e:
                            if e.resp.status == 410:
                                # Sync token expired, do full sync
                                logger.info(
                                    f"Sync token expired for {calendar_id}, doing full sync"
                                )
                                sync_token = None
                            else:
                                raise
                    ...
                    if new_sync_token:
                        self.db.update_sync_metadata(
                            f"gcal_{calendar_id}", success=True, sync_token=new_sync_token
                        )
                    else:
                    ...
                    self.db.update_sync_metadata(
                        f"gcal_{calendar_id}", success=False, error_message=str(e)
                    )
